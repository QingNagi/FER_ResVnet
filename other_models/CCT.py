...
class Conv_Block(torch.nn.Module):
    ...
    def seq_len(self):
        return self.forward(torch.zeros(1, 1, self.img_size, self.img_size)).shape[2]
...
class Multi_Head_ATT(torch.nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        if self.head == 1:
            ...
            k = self.k_att(k).view(-1, seq_len, self.head, self.emb_dim // self.head).permute(0, 2, 1, 3).permute(0, 1,
                                                                                                                  3, 2)
...
class Encoder(torch.nn.Module):
    def __init__(self, num_layer, emb_dim, head, dim_expan=4, dropout=0, layer_drop=0):
        ...
        self.connect1 = torch.nn.ModuleList([
            Multi_Head_ATT(emb_dim, multi_head=head, dropout=dropout, layer_drop=layer_drop) for i in
            range(num_layer - 1)
        ])
        self.connect2 = torch.nn.ModuleList([
            Feed_Forward(emb_dim, dim_expan=dim_expan, dropout=dropout, layer_drop=layer_drop) for i in
            range(num_layer - 1)
        ])
...
class CCTransformer(torch.nn.Module):
    def __init__(self,
        ...
        self.conv_blk = Conv_Block(conv_layer, img_size, in_channel, emb_dim, k_size, stride,
                                   padding)  # num_layer, img_size, in_channel, emb_dim, k_size, stride, padding, position = True
        ...
        self.encoder = Encoder(num_layer, emb_dim[-1], head, dim_expan=dim_expan, dropout=dropout,
                               layer_drop=layer_drop)  # num_layer, seq_len, emb_dim, head
