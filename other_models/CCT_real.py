...
from CCTtransformers import TransformerClassifier
...
class CCT(nn.Module):
    ...
    def forward(self, x):
        x = self.tokenizer(x)
        return self.classifier(x.clone())
...
@register_model
def cct_7_cbam_resnet(pretrained=False, progress=False,
                 img_size=32, positional_embedding='learnable', num_classes=10,
                 *args, **kwargs):
    return cct_7('cct_7_3x1_32', pretrained, progress,
                 kernel_size=3, n_conv_layers=1,
...
model = cct_7_cbam_resnet().to('cpu')
print(model)
